 ...
 * ====================== */
namespace library;
use library\upload\Drive\Alioss;
use library\upload\Compress;
/**
 * $conf['path']		= './uploads/img';			// 上传目录
 * $conf['allowType']	= array('jpg','gif','png');	// 允许的文件后缀
 * $conf['maxSize']		= 1024*1024;				// 允许的文件大小
 * $conf['compress']	= true;						// 是否开启图片压缩
 * $conf['percent']		= 1;						// 图片压缩比
 * 
 * $up     			= new \Library\upload($conf);
 * $result 			= $up->upload(true);    // 参数true遇到错误继续，返回上传的文件信息，键名对应表单的name值
 * $info   			= $up->getInfo();       // 返回上传文件信息，索引数组
 * $compressinfo	= $up->getCompress(array(array('width'=>'200','height'=>'200'),array('width'=>'100','height'=>'100')));	// 返回压缩图片数组信息
...
class Upload {
	...
	private $compress = false;// 是否开启图片压缩
	private $percent = 0; // 图片压缩比
	...
	*/
	private function getFileName($i){
		...
		if(!$this->randName){
			...
			return uniqid()."{$i}.{$this->filesInfo['suffix'][$i]}";
		}
	}
	/**
	* 创建目录
	...
	*/
	private function moveFile($i){
		...
		if($mov){
			...
			if($this->compress){
				// 单图片压缩
				//(new Compress($this->mapping[$i]['path'],$this->percent))->compressImg($this->mapping[$i]['path']);
			}
			// 判斷是否開啟OSS
			if(C('ALIOSS_AUTO')){
				$oss = new Alioss();
				$ossdata = $oss->upload($this->mapping[$i]['path']);
				if($ossdata['err']){
					$this->error[] = $ossdata['err'];
					return false;
				}else{
					$this->mapping[$i]['path'] = $ossdata['path'];
				}
			}
			return true;
		}else{
	...
	// 压缩图片数组信息
	private function getCompress($arr){
		// 判斷是否開啟图片压缩
		if($this->compress){
			p($this->mapping[0]['path']);
			die();
			// 单图片压缩
			$data = [];
			foreach($arr as $key => $value){
				//$data[$key]['url'] = (new Compress($this->mapping[0]['path'], $this->percent, $value['width'], $value['height']))->compressImg($this->['path'] . '/' . $value['width'] . '_' . $value['height']);
			}
			return $data;
		}else{
			return null;
